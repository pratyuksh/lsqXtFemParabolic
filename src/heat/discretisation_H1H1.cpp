                               ...
                               std::shared_ptr<Mesh>& xMesh)
{
    ...
    std::cout << "\tNumber of degrees of freedom"
         ...
         << xdimV2 << std::endl;
#endif
    m_xFecs.Append(xH1_coll1);
    m_xFecs.Append(xH1_coll2);
    
    m_xFespaces.Append(xV1space);
    m_xFespaces.Append(xV2space);
    ...
    if (xMesh->bdr_attributes.Size())
    {
        ...
        xV1space->GetEssentialTrueDofs(m_xEssBdrMarker,
                                       m_xEssTdofList);
...
void heat::LsqXtFemH1H1 :: assembleSystemMediumIndependent()
{
    ...
    xStiff2_form->AddDomainIntegrator
            (new heat::VectorStiffnessIntegrator);
...
void heat::LsqXtFemH1H1 :: assembleSystemMediumDependent()
{
    ...
    xGrad_form->AddDomainIntegrator
            (new heat::VectorGradientIntegrator(&medCoeff));
    xGrad_form->Assemble();
    xGrad_form->Finalize();
...
:: assembleSourceXDiv(Vector& b, double t) const
{
    ...
    source_form.AddDomainIntegrator
            (new heat::VectorDivergenceLFIntegrator(f,-1));
